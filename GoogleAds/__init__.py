...
def fetch_gads_campaign_performance_data(cid, year, month, from_day, to_day):
    ...
    from_day_formatted = int(from_day) - 1
    to_day_formatted = int(to_day) + 1

    campaign_performance_data = []

    # 기기는 enum 데이터이며, PC => 모바일 => 태블릿 => 기타 순
    for j in days31[from_day_formatted:to_day_formatted]:
        query = f"""
                SELECT
                    campaign.id,
                    campaign.name,
                ...
                FROM campaign
                WHERE
                    segments.date BETWEEN '{year + month + from_day}' AND '{year+ month+to_day}'
                """
        ...
        for batch in stream:
            for row in batch.results:
                ...
                campaign_performance_data.append({
                        ...
                        "ctr": (metrics.clicks/metrics.impressions) if not metrics.impressions == 0 else 0,
                        "ccnt": int(metrics.conversions),
                        "crto": (metrics.conversions/metrics.clicks) if not metrics.clicks == 0 else 0,
